...
namespace Conduit.DependencyInjection.Tests;

public sealed class AddConduitTests
{
    ...
    private sealed class TestDispatcher : ISender, IPublisher
    {
        ...
        public ValueTask<TResponse> Send<TResponse>(IRequest<TResponse> request, CancellationToken cancellationToken = default)
        {
            if (request is Ping ping && typeof(TResponse) == typeof(string))
            {
                ...
                return Adapt<string, TResponse>(Conduit.Pipeline.PipelineComposer.ExecuteAsync(ping, behaviors, handler.Handle, cancellationToken));
            }
    ...
    [Fact]
    public void AddConduitCore_Registers_Options()
    {
        ServiceCollection services = new();
        services.AddConduitCore(o => o.DefaultHandlerLifetime = ServiceLifetime.Singleton);

        using ServiceProvider sp = services.BuildServiceProvider();
        ConduitOptions options = sp.GetRequiredService<ConduitOptions>();
    ...
    public async Task Behaviors_Are_Resolved_And_Executed_In_Registration_Order()
    {
        ...
        services.AddConduitDispatcher<TestDispatcher>();
    ...
    [Fact]
    public async Task AddConduit_With_Manual_Dispatcher_And_Handler_Works()
    {
        ServiceCollection services = new();
        services.AddConduitCore();
        services.AddScoped<IRequestHandler<Ping, string>, PingHandler>();
    ...
    [Fact]
    public void AddConduit_Invokes_Generated_Bridge_When_Set()
    {
        bool called = false;
        ConduitGeneratedRegistrationBridge.Register = (services, options) =>
        {
            called = true;
        ...
        {
            ConduitGeneratedRegistrationBridge.Register = null;
        }
    }

    [Fact]
    public void ConduitOptions_GlobalBehaviors_Is_Mutable_List()
    {
